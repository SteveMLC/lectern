...
| One-way Accelevents integration | `/admin/integrations` | n/a | Setup required | The existing event-scoped connection record is now visible with direction, state, secret boundary, and official credential path. The seeded connection remains `awaiting_credentials`; no unverified external push is claimed. |
