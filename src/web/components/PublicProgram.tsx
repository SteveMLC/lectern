      ...
      <div className="p-5">
        {selectedSession ? (
          ...
          <div>
            ...
            <div className="mt-2 divide-y divide-zinc-100">{speakerSessions.map((session) => { const slot = schedule.slots.find((candidate) => candidate.session.id === session.id) ?? null; return <button key={session.id} type="button" className="group block w-full py-3 text-left" onClick={() => { setSelectedSpeakerId(null); setSelectedSessionId(session.id); setView("sessions"); }}><span className="block text-sm font-medium text-zinc-900 group-hover:text-accent">{session.title} →</span><span className="mt-1 block text-xs text-zinc-500">{slot ? `${formatDay(localDay(slot.startsAt, schedule.timezone))} · ${formatTimeRange(slot.startsAt, slot.endsAt, schedule.timezone)} · ${slot.room?.name ?? "Room pending"}` : "Time and room coming soon."}</span></button>; })}</div>
          </div>
        ) : view === "schedule" ? (
          <div>
