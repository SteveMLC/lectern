...
For an AI provider invoice or subscription receipt, explicitly copy the raw file here and run `pnpm usage:receipt -- ...`. The command refuses files elsewhere (and symlinks that resolve elsewhere). The tracked receipt allocation contains only its SHA-256, byte size, billed amount, period, and covered usage-entry ids. Do not edit old ledger entries or commit the private source file.
